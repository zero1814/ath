	...
	<div id="wrapper">
						...
						<c:choose>
							<c:when test="${fn:length(group.menus)>0}">
								<li class="hidden-folded padder m-t m-b-sm text-muted text-xs">
									<span class="ng-scope">${group.name}</span>
									<c:forEach var="menu" items="${group.menus }">
										<c:choose>
							                            	...
							                            	<c:forEach var="childMenu" items="${menu.childMenu }">
																<c:choose>
																	<c:when test="${fn:length(childMenu.childMenu)>0}">
																		<li>
																			<a href="#">${childMenu.name} <span class="fa arrow"></span></a>
												                        </li>
												                        <ul class="nav nav-third-level">
																			<c:forEach var="item" items="${childMenu.childMenu}">
											                                    <li>
											                                    	<a class="J_menuItem" href="${item.url }">${item.name }</a>
											                                    </li>
																			</c:forEach>
												                        </ul>
																	</c:when>
		...
		<div id="page-wrapper" class="gray-bg">
			<div class="row J_mainContent" id="content-main">
				<iframe id="J_iframe" width="100%" height="100%" src="index2.htm"
					frameborder="0" data-id="index2.htm" seamless></iframe>
			</div>
		</div>
	</div>
</body>
