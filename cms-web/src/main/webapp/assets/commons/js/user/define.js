				...
				success : function(result) {
					if (result.code == 0) {
						...
						Tree.init("treeview",false,tree,function(event, node) {
							if(node.code){
								if(node.code == "0"){
									var url = "user/define/addindex.htm?code=0";
			...
			$.ajax({
				...
				success: function(result) {
					layer.alert(result.message, function() {
						layer.close(Define.Layer);
						window.parent.location.href="user/define/index.htm?groupCode="+groupCode;
					});
				},
				error: function(result) {
					layer.alert('添加失败，失败原因:' + JSON.stringify(result),function(index){
						layer.close(index);
						window.parent.location.href="user/define/index.htm?groupCode="+$("#groupCode").val();
					});
				}
			});
		},
					...
					success: function(result) {
						if(result.code == 0) {
							layer.alert('删除成功', function(index) {
								layer.close(index);
								window.parent.location.href="user/define/index.htm?groupCode="+group;
							});
						} else {
							layer.alert(result.message, function(index) {
								layer.close(index);
