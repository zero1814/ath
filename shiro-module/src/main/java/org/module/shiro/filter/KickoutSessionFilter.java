...
public class KickoutSessionFilter extends AccessControlFilter {
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue)
		...
		if (null != marker && marker) {
			...
			if (ShiroFilterUtils.isAjax(request)) {
				LoggerHelper.debug(getClass(), "当前用户已经在其他地方登录，并且是Ajax请求！");
		 ...
		 */
		if (infoMap.containsKey(userCode) && !infoMap.containsValue(sessionId)) {
			Serializable oldSessionId = infoMap.get(userCode);
			...
			if (null != oldSession) {
				...
				LoggerHelper.debug(getClass(), "kickout old session success,oldId:" + oldSessionId);
			} else {
				shiroSessionRepository.deleteSession(oldSessionId);
				infoMap.remove(userCode);
		...
		if (!infoMap.containsKey(userCode) && !infoMap.containsValue(sessionId)) {
			infoMap.put(userCode, sessionId);
			// 存储到缓存1个小时（这个时间最好和session的有效期一致或者大于session的有效期）
			cache.setex(ONLINE_USER, infoMap, 3600);
		}
		return Boolean.TRUE;
	...
	private void out(ServletResponse hresponse, Map<String, String> resultMap) throws IOException {
		try {
			...
			LoggerHelper.error(getClass(), "KickoutSessionFilter.class 输出JSON异常，可以忽略。");
		}
	}
