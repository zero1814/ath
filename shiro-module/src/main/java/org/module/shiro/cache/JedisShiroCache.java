...
/**
 * 
 * 类: JedisShiroCache <br>
 * 描述: 缓存获取Manager <br>
 * 作者: zhy<br>
 * 时间: 2017年7月3日 上午10:30:23
 * 
 * @param <K>
 * @param <V>
 */
...
public class JedisShiroCache<K, V> implements Cache<K, V> {
	...
	public V get(K key) throws CacheException {
		...
		try {
			...
			LoggerHelper.error(SELF, "get value by cache throw exception");
		}
	...
	public V put(K key, V value) throws CacheException {
		V previos = get(key);
		try {
			jedisManager.saveValueByKey(SerializeUtil.serialize(buildCacheKey(key)), SerializeUtil.serialize(value),
					-1);
		} catch (Exception e) {
			LoggerHelper.error(SELF, "put cache throw exception");
		}
		return previos;
	}
	...
	public V remove(K key) throws CacheException {
		...
		try {
			...
			LoggerHelper.error(SELF, "remove cache  throw exception");
		}
		return previos;
	}
